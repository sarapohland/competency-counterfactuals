...
class PARCE:
    ...
    # Estimate probabilistic perception model competency with PyTorch
    def comp_scores_torch(self, inputs, outputs, zscore=None):
        # Select z score for competency estimation
        zscore = zscore if zscore is not None else self.zscore
        ...
        # Estimate probability that image is ID
        stds = torch.tensor(self.class_stds, device=outputs.device, dtype=outputs.dtype)
        means = torch.tensor(self.class_means, device=outputs.device, dtype=outputs.dtype)
        losses_arr = losses.unsqueeze(1).expand(-1, self.C)
        zvals = (losses_arr - 2 * means) / stds - zscore
        loss_probs = 1 - torch.distributions.Normal(0, 1).cdf(zvals)

        # Estimate probability that model is competent
        scores = torch.sum(loss_probs * class_probs, dim=1)
        if self.method == 'overall':
            scores *= torch.max(class_probs, dim=1).values
